...
TEST_CASE_FIXTURE(MessageDigestTest, "MessageDigestTest.testDigest")
{
  md5_->update("aria2", 5);
  ...
  REQUIRE_EQ(std::string("05e101d0"), util::toHex(adler32_->digest()));

  adler32_->reset();
  adler32_->update("abc", 3);
  REQUIRE_EQ(std::string("024d0127"), util::toHex(adler32_->digest()));
#endif // HAVE_ZLIB
}

TEST_CASE_FIXTURE(MessageDigestTest, "MessageDigestTest.testSupports")
{
...
TEST_CASE_FIXTURE(MessageDigestTest, "MessageDigestTest.testGetDigestLength")
{
...
TEST_CASE_FIXTURE(MessageDigestTest, "MessageDigestTest.testIsStronger")
{
...
TEST_CASE_FIXTURE(MessageDigestTest, "MessageDigestTest.testIsValidHash")
{
...
TEST_CASE_FIXTURE(MessageDigestTest,
                  "MessageDigestTest.testGetCanonicalHashType")
{
  REQUIRE_EQ(std::string("sha-1"), MessageDigest::getCanonicalHashType("sha1"));
