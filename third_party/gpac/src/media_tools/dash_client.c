...
struct __dash_group
{
	...
	Bool resume_requested, resume_pending;
	u64 resume_time_ms;
...
static GF_Err gf_dash_get_date(GF_DashClient *dash, char *scheme_id, char *url, u64 *utc, u64 *sample_time)
{
	GF_Err e;
	u8 *data = NULL;
	u32 len = 0;
	...
	*utc = 0;
	...
	/* The I/O adapter may store responses on disk or in a native blob. */
	gf_file_load_data(cache_name, &data, &len);
	...
	*sample_time = dash->dash_io->get_utc_start_time
		? dash->dash_io->get_utc_start_time(dash->dash_io, dash->pending_utc_session) : gf_net_get_utc();
	gf_free(data);
...
setup_multicast_clock:
	...
	if (utc_timing && utc_timing->scheme_id_uri
		...
		u64 utc=0, sample_time=fetch_time;
		s64 drift_estimate;
		...
		if (!strcmp(utc_timing->scheme_id_uri, "urn:mpeg:dash:utc:direct:2014")) {
			...
			while (time_refs) {
				...
				if (group->dash->pending_utc_idx>=utc_idx) {
					res = gf_dash_get_date(group->dash, utc_timing->scheme_id_uri, time_refs, &utc, &sample_time);
					if (res==GF_NOT_READY) {
	...
	if (group->dash->resume_requested && group->dash->initial_period_tunein)
		current_time = group->dash->resume_time_ms;
	...
	if (timeline) {
		...
		u64 last_s_dur=0;
		u32 i, seg_idx = 0;
		gf_dash_get_segment_duration(rep, group->adaptation_set, group->period, mpd,
			&group->nb_segments_in_rep, &group->segment_duration);
		...
		for (i=0; i<count; i++) {
			...
			if (!segtime) {
				...
				// A Period can start just before its first audio sample.
...
restart_period_check:
	...
	if (!new_period->duration) {
		GF_MPD_Period *next_period = gf_list_get(new_mpd->periods, dash->active_period_index+1);
		if (next_period && next_period->start > new_period->start)
			new_period->duration = next_period->start - new_period->start;
	}
...
process_m3u8_manifest:
	...
	for (group_idx=0; group_idx<gf_list_count(dash->groups); group_idx++) {
		...
		/* Fixed HLS downloads only refresh selected renditions. */
		if (group->selection==GF_DASH_GROUP_NOT_SELECTABLE)
			continue;
		if (dash->is_m3u8 && dash->disable_switching && dash->period_groups_setup
...
//#define DUMP_LIST
	...
	for (group_idx=0; group_idx<gf_list_count(dash->groups) && new_period; group_idx++) {
		...
		if (seg_dur)
			group->segment_duration = seg_dur;
...
static GF_Err gf_dash_resolve_url(GF_MPD *mpd, GF_MPD_Representation *rep, GF_DASH_Group *group, const char *mpd_url, GF_MPD_URLResolveType resolve_type, u32 item_index, char **out_url, u64 *out_range_start, u64 *out_range_end, u64 *segment_duration, Bool *is_in_base_url, char **out_key_url, bin128 *out_key_iv, Bool *data_url_process, u32 *out_start_number)
{
	...
	if (!group->dash->is_m3u8 && mpd->type==GF_MPD_TYPE_DYNAMIC) {
		u64 duration = 0;
		u32 scale = 1;
		GF_MPD_SegmentTimeline *timeline = NULL;
		gf_mpd_resolve_segment_duration(rep, set, period, &duration, &scale, NULL, &timeline);
		if (timeline && !gf_list_count(timeline->entries)) {
			group->dash->force_mpd_update = GF_TRUE;
			dash_set_min_wait(group->dash, mpd->minimum_update_period ? mpd->minimum_update_period : 1000);
			return GF_NOT_READY;
		}
	}
	...
	if (group->resume_pending) {
		u64 duration = 0, pto = 0, target;
		u32 scale = 1;
		GF_MPD_SegmentTimeline *timeline = NULL;
		gf_mpd_resolve_segment_duration(rep, set, period, &duration, &scale, &pto, &timeline);
		target = gf_timestamp_rescale(group->resume_time_ms, 1000, scale);
		if (timeline) {
			target += pto;
			group->download_segment_index = gf_dash_get_index_in_timeline(timeline, target, scale, scale);
		} else if (duration) {
			group->download_segment_index = (u32) ((target + duration - 1) / duration);
		} else {
			return GF_NOT_SUPPORTED;
		}
		item_index = group->download_segment_index;
		group->resume_pending = GF_FALSE;
	}
...
static void gf_dash_cache_segment_info(GF_DASH_Group *group, GF_MPD_Representation *rep, segment_cache_entry *entry, u32 start_number)
{
	...
	entry->duration = duration && scale ? (u32) gf_timestamp_rescale(duration, scale, 1000)
		: (u32) group->current_downloaded_segment_duration;
...
retry_rep:
	...
	for (i=0; i<gf_list_count(group->adaptation_set->representations); i++) {
		...
		/* WebM SegmentBase addresses a complete representation. Its Cue index
		   is not an ISO BMFF SIDX; the native demuxer reads the complete resource. */
		const char *mime = rep->mime_type ? rep->mime_type : group->adaptation_set->mime_type;
		if (mime && (!stricmp(mime, "video/webm") || !stricmp(mime, "audio/webm")
			|| !stricmp(mime, "video/x-matroska") || !stricmp(mime, "audio/x-matroska"))) {
			GF_MPD_SegmentBase *base = rep->segment_base ? rep->segment_base :
				(group->adaptation_set->segment_base ? group->adaptation_set->segment_base : group->period->segment_base);
			GF_MPD_SegmentURL *segment;
			GF_SAFEALLOC(rep->segment_list, GF_MPD_SegmentList);
			GF_SAFEALLOC(segment, GF_MPD_SegmentURL);
			if (!rep->segment_list || !segment) { gf_free(segment); return GF_OUT_OF_MEM; }
			rep->segment_list->timescale = 1000;
			rep->segment_list->duration = group->period->duration ? group->period->duration : group->dash->mpd->media_presentation_duration;
			if (base) rep->segment_list->presentation_time_offset = gf_timestamp_rescale(base->presentation_time_offset, base->timescale ? base->timescale : 1, 1000);
			rep->segment_list->segment_URLs = gf_list_new();
			gf_list_add(rep->segment_list->segment_URLs, segment);
		...
		if (is_isom && (init_in_base || index_in_base)) {
			...
			/*we need to download the init segment, at least partially*/
				...
				/* A declared SIDX range already bounds the initialization prefix. */
				if (index_in_base && index_end_range) {
					e = gf_dash_download_resource(group->dash, download_sess, init_url, 0, index_end_range, 1, group);
					group->sidx_start = index_start_range;
					group->box_type = GF_ISOM_BOX_TYPE_SIDX;
				} else {
...
retry_pending_rep:
		...
		if (dash->start_range_period) {
			gf_dash_seek_group(dash, group, dash->start_range_period,
				(dash->mpd->type==GF_MPD_TYPE_DYNAMIC) ? GF_TRUE : GF_FALSE);
		}
...
static DownloadGroupStatus on_group_download_error(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group, GF_Err e, GF_MPD_Representation *rep, char *new_base_seg_url, char *key_url, Bool has_dep_following)
{
	...
	if (!will_retry) {
		if (is_live && dash->disable_switching)
			dash->dash_io->on_dash_event(dash->dash_io, GF_DASH_EVENT_TIMESHIFT_OVERFLOW, 1, e);
		...
		if (group->download_segment_index  >= (s32) group->nb_segments_in_rep) {
			...
			if (dash->mpd->minimum_update_period || dyn_type==GF_MPD_TYPE_DYNAMIC) {
				...
				if (! group->maybe_end_of_stream) {
					...
					if (!dash->is_m3u8 && (timer < dash->mpd->minimum_update_period))
						dash_set_min_wait(dash, dash->mpd->minimum_update_period - timer);
					...
					if (dash->is_m3u8 && (dyn_type==GF_MPD_TYPE_DYNAMIC)) {
						if (!group->is_low_latency && rep->segment_list && rep->segment_list->timescale) {
							u32 delay = (u32) gf_timestamp_rescale(rep->segment_list->duration, rep->segment_list->timescale, 500);
							u32 elapsed = now - dash->last_update_time;
							if (elapsed < delay) {
								dash_set_min_wait(dash, delay - elapsed);
								return GF_DASH_DownloadCancel;
							}
						}
...
static GF_Err dash_setup_period_and_groups(GF_DashClient *dash)
{
	...
	if (!dash->period_groups_setup) {
		...
		e = dash->dash_io->on_dash_event(dash->dash_io, GF_DASH_EVENT_SELECT_GROUPS, -1, GF_OK);
		if (e) return e;
	...
	for (i=0; i<group_count; i++) {
		...
		/* Apply an explicit startup selection before fetching initialization or
		   the first media segment. Runtime switching is otherwise deferred. */
		if (group->force_representation_idx_plus_one)
			gf_dash_switch_group_representation(dash, group);
		...
		if (dash->initial_period_tunein && !dash->mcast_clock_state && !group->resume_requested) {
			group->timeline_setup = GF_FALSE;
			group->force_timeline_reeval = GF_TRUE;
		}
		if (e) break;
...
GF_Err gf_dash_group_resume_sequence(GF_DashClient *dash, u32 group_idx, u32 sequence)
{
	...
	GF_MPD_Representation *rep;
	u32 i, count;
	if (!group || !dash->is_m3u8 || group->group_setup || group->nb_cached_segments)
		return GF_BAD_PARAM;
	if (group->force_representation_idx_plus_one)
		gf_dash_switch_group_representation(dash, group);
	rep = gf_list_get(group->adaptation_set->representations, group->active_rep_index);
	if (!rep || !rep->segment_list) return GF_BAD_PARAM;
	count = gf_list_count(rep->segment_list->segment_URLs);
	for (i=0; i<count; i++) {
		GF_MPD_SegmentURL *segment = gf_list_get(rep->segment_list->segment_URLs, i);
		if (segment->hls_seq_num < sequence) continue;
		if ((u64) segment->hls_seq_num > (u64) sequence+1) return GF_URL_REMOVED;
		group->download_segment_index = i;
		return GF_OK;
	}
...
GF_EXPORT
GF_Err gf_dash_group_resume_time(GF_DashClient *dash, u32 group_idx, u64 time_ms)
{
	GF_DASH_Group *group = gf_dash_get_active_group(dash, group_idx);
	if (!group || dash->is_m3u8 || group->group_setup || group->nb_cached_segments)
		return GF_BAD_PARAM;
	group->resume_time_ms = time_ms;
	group->resume_requested = group->resume_pending = GF_TRUE;
	return GF_OK;
}

GF_EXPORT
GF_Err gf_dash_resume_at(GF_DashClient *dash, u64 time_ms)
{
	GF_MPD_Period *first;
	if (!dash || !dash->mpd || dash->is_m3u8 || dash->period_groups_setup)
		return GF_BAD_PARAM;
	first = gf_list_get(dash->mpd->periods, 0);
	if (!first || time_ms < first->start) return GF_URL_REMOVED;
	dash->resume_requested = GF_TRUE;
	dash->resume_time_ms = time_ms;
	return GF_OK;
}
...
const char *gf_dash_group_get_segment_init_keys(GF_DashClient *dash, u32 idx, u32 *crypt_type, bin128 *key_IV)
{
	...
	if (crypt_type) {
		*crypt_type = (rep->crypto_type > 1 || group->init_segment_is_media)
			? rep->crypto_type : (rep->playback.key_url ? 1 : 0);
	}
